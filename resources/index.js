...
const inputSelectDino = document.querySelector('#select-dino')
...
dinos.forEach(dino =>{
    ...
    id.textContent = dinos.id
    const name = document.createElement('name')
    name.textContent = dinos.name
    const img = document.createElement('img')
    img.src = dinos.img
    const cena = document.createElement('cena')
    inputSelectDino.append(id, name, img, cena)
})


inputForma.addEventListener('submit', e =>{
    e.preventDefault
    let i = 0
    i++
    inputForma.textContent
